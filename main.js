...
function renderLanding() {
  ...
  if (!sessionStorage.getItem('landingViewed')) {
    sendAnalyticsEvent('7193_page_view_var8', '7193_page_view_var8');
    sessionStorage.setItem('landingViewed', '1');
  }
  // Если уже была заглушка для этого варианта, не показываем лендинг
  if (localStorage.getItem('7193_placeholderShown_var8') === '1') {
    renderPlaceholder();
    return;
  }
  app.innerHTML = `
      ...
      <div class="landing__header">
        <div class="landing__header-content">
          ...
          <div class="landing__header-bonuses">
            <div class="landing__bonus"><span style="white-space: nowrap;">Вам: 300 ₽</span></div>
  ...
  document.getElementById('sendBtn').onclick = () => {
    sendAnalyticsEvent('7193_click_continue_var8', '7193_click_continue_var8');
    localStorage.setItem('7193_placeholderShown_var8', '1');
    renderPlaceholder();
...
function renderPlaceholder() {
  ...
  if (!sessionStorage.getItem('endPageViewed')) {
    sendAnalyticsEvent('7193_end_page_view_var8', '7193_end_page_view_var8');
