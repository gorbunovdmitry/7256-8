...
function renderLanding() {
  ...
  if (!sessionStorage.getItem('landingViewed')) {
    sendAnalyticsEvent('7256_page_view_var8', '7256_page_view_var8');
    sessionStorage.setItem('landingViewed', '1');
  }
  // Если уже была заглушка для этого варианта, не показываем лендинг
  if (localStorage.getItem('7256_placeholderShown_var8') === '1') {
    renderPlaceholder();
    return;
  }
  app.innerHTML = `
  ...
  document.getElementById('sendBtn').onclick = () => {
    sendAnalyticsEvent('7256_click_continue_var8', '7256_click_continue_var8');
    localStorage.setItem('7256_placeholderShown_var8', '1');
    renderPlaceholder();
...
function renderPlaceholder() {
  ...
  if (!sessionStorage.getItem('endPageViewed')) {
    sendAnalyticsEvent('7256_end_page_view_var8', '7256_end_page_view_var8');
